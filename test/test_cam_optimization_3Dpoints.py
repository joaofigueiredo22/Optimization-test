...
def objective_functionv2(params, points_chessboard):
    ...
    for i in range(dimensions[0] * dimensions[1]):
        points_op = np.matmul(np.array(right_cam_T_chess_opt), points_chessboard_homogeneous[:, i].reshape((4, 1)))
        points_real_right = np.matmul(right_cam_T_chess_ground_truth, points_chessboard_homogeneous[:, i].reshape((4, 1)))
        residuals[i] = np.sqrt(
            (points_op[0] - points_real_right[0]) ** 2 + (points_op[1] - points_real_right[1]) ** 2 + (
                        points_op[2] - points_real_right[2]) ** 2)
